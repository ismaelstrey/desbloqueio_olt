      ...
      >
        ...
        <div className="grid md:grid-cols-[300px,1fr] gap-8">
              ...
              {docs.map((doc) => (
                ...
                >
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 rounded-lg transition-colors ${selectedDoc?.name === doc.name ? 'bg-[#F3F821] text-black' : 'hover:bg-gray-700'}`}
                  >
                    {doc.name}
                  </button>
                </motion.li>
          ...
          <motion.div
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            className="bg-gray-800 p-6 rounded-lg shadow-xl overflow-auto"
          >
            {loading ? (
